"""Database startup: wait for Postgres and create tables (Render / Docker)."""
import sys
import time
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            with app.app_context():
                ...
                db.session.commit()
                initialize_schema()
            print("Database connection OK — tables ready.", flush=True)
            return 0
        except OperationalError as exc:
            print(f"Waiting for database ({attempt}/{MAX_RETRIES}): {exc}", flush=True)
        except Exception as exc:
            print(f"Database init error ({attempt}/{MAX_RETRIES}): {exc}", flush=True)
            db.session.rollback()
        time.sleep(RETRY_DELAY)

    print("Could not initialize the database.", file=sys.stderr, flush=True)
    return 1
